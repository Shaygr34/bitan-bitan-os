 ...
 * timeout (60s), and retry on 429 (exponential backoff, max 3 attempts).
 */
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        console.warn(`Claude API rate limited (attempt ${attempt + 1}), retrying in ${backoff}ms`);
        await sleep(backoff);
        continue;
      }
      ...
      if (attempt < MAX_RETRIES) {
        ...
        console.warn(`Claude API error (attempt ${attempt + 1}), retrying in ${backoff}ms:`, (err as Error).message);
        await sleep(backoff);
      }
    }
  }
